! Input parameters: N= total number of mutations, in main program.
! Distribution of r, m: as mentioned in article. 
! Output: three columns .
! Column 1: x (rescaled concentration)
! Column 2: mean number of mutations in fitnesspeak
! Column 3: mean number of fitness peaks
...
program main
  ...
  integer,parameter::N=12,NN=20,kexp=1
  ...
  integer::str(1:N),seed=-78974946,npk=0,nstr(1:N)=0,ft=0
  double precision::rw=1.d0,rsingle(1:N)=0.d0,micw=1.d0,micsingle(1:N)=0.d0,r0(0:2**N-1)=1.d0,r(0:2**N-1)=0.d0
  double precision::xu=0.d0,xl=0.d0,dx=.001d0,rnum2=0.d0,cs2=0.d0,kn(1:NN,0:N)=0.d0,logmu=0.d0,xx(1:NN)=0.d0,rfittest=0.d0
...
double precision::gam=0.d0,alph=1.d0,pktl(1:NN)=0.d0,sz2=0.d0,szav(1:NN)=0.d0,szft(1:NN,1:2**n)=0.d0,ct(1:NN,1:2**n)=0.d0
integer::peakrank(1:2**N)=0,pks=0,pkx=0,pmax=0,ij,jk
...
60 format(f48.9,3x,f16.7,3x,f16.7,3x,f16.7)
...
str=bitstr(j,N)
   ...
   if (npk==N) then
      ...
      cs2=cs2+size(str,N)
      sz2=sz2+size(str,N)
      szav(kk)=szav(kk)+size(str,N)
      kn(kk,szint)=kn(kk,szint)+1.d0
      if (r(j)> rfittest) ft=j
   ...
   if (pmax.ge.2) then
        do ij=1,pmax
           do jk=1,pmax-1
              if (r(peakrank(jk))<r(peakrank(jk+1))) then
                 pkx=peakrank(jk)
                 peakrank(jk)=peakrank(jk+1)
                 peakrank(jk+1)=pkx
              endif
           enddo
        enddo
        ...
        do ij=1,2**n
           if (pmax.ge.ij) then
              szft(kk,ij)=szft(kk,ij)+size(bitstr(peakrank(ij),N),N)
 ct(kk,ij)=ct(kk,ij)+1.d0
endif
enddo
...
do kk=1,NN
   do ij=1,2**n
      szft(kk,ij)=szft(kk,ij)/(ct(kk,ij))
  enddo
enddo


do kk=1,NN
print*, xx(kk),szft(kk,1),pktl(kk)
enddo
